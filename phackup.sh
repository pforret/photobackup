...
### 2. implement the different actions in Script:main() directly or with helper functions do_backup
### 3. implement helper functions you defined in previous step
### ==============================================================================

### Created by Peter Forret ( pforret ) on 2025-10-26
...
readonly script_created="2025-10-26"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="Photo backup script"
...
choice|1|action|action to perform|backup,check,env,update
" -v -e '^#' -e '^\s*$'
}
...
function Script:main() {
  ...
  backup)
    ...
    [[ -z "$SOURCE" ]]   && IO:die "SOURCE folder not specified, use -S|--SOURCE to specify it"
    [[ ! -d "$SOURCE" ]] && IO:die "SOURCE folder [$SOURCE] not found"
    [[ -z "$DESTIN" ]]   && IO:die "DESTIN folder not specified, use -D|--DESTIN to specify it"
    [[ ! -d "$DESTIN" ]] && IO:die "DESTIN folder [$DESTIN] not found (maybe it has to be mounted first?)"
    local tmp_file
